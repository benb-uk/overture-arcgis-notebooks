...
[Overture Maps Foundation](https://overturemaps.org) place data, licensed under [CDLA Permissive 2.0](https://cdla.dev/permissive-2-0/). Queried directly from `s3://overturemaps-us-west-2`.

Street network routing via the ArcGIS Logistics OD Cost Matrix service, powered by HERE.

## Usage

1. Open the notebook in ArcGIS Online Notebooks
2. Run all cells in order
3. Adjust `xmin`, `ymin`, `xmax`, `ymax` to change the area of interest
4. Adjust `STATION_X`, `STATION_Y` and the origin name to change the origin point
5. Adjust the category filter in the SQL query to change the place type

## Notes

- The Overture release version is set in `RELEASE` — update this to the latest release as needed. Current releases are listed at [overturemaps.org](https://overturemaps.org)
